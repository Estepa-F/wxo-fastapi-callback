...
# App
# ==================================================
app = FastAPI(title="WXO Async Image -> COS -> Presigned URL", version="1.0.0")
...
def make_object_key(job_id: str, filename: Optional[str]) -> str:
    # On force PNG car le stub ressort du PNG.
    # Si plus tard tu gardes le format d'origine, adapte ici.
    base = filename or "image"
    return f"results/{job_id}/{base}.png"
    ...
    try:
        url = s3.generate_presigned_url(
            ...
            Params={
                "Bucket": COS_BUCKET,
                "Key": object_key,
                # Optionnel: forcer le téléchargement
...
# ==================================================
# Background job: decode -> process -> COS -> callback
# ==================================================
